### Hi there 👋

#### 👷 Check out what I'm currently working on

- [docker/packaging](https://github.com/docker/packaging) - Docker Packaging (deb, rpm, static) (1 day ago)
- [crazy-max/docker-matomo](https://github.com/crazy-max/docker-matomo) - Matomo (formerly Piwik) Docker image (1 day ago)
- [docker/setup-docker-action](https://github.com/docker/setup-docker-action) - GitHub Action to set up (download and install) Docker CE (1 day ago)
- [docker/github-builder-experimental](https://github.com/docker/github-builder-experimental) - Official Docker-maintained reusable GitHub Actions workflows (1 day ago)
- [crazy-max/docker-unbound](https://github.com/crazy-max/docker-unbound) - Unbound Docker image (2 days ago)

#### 👨‍💻 Repositories I created recently

- [crazy-max/buildx-3338](https://github.com/crazy-max/buildx-3338)
- [crazy-max/docker-scrutiny](https://github.com/crazy-max/docker-scrutiny)
- [crazy-max/test-builder-action](https://github.com/crazy-max/test-builder-action)
- [crazy-max/builder-action](https://github.com/crazy-max/builder-action)
- [crazy-max/docker-captains-briefing-20250129](https://github.com/crazy-max/docker-captains-briefing-20250129)

#### 🚀 Latest releases I've contributed to

- [docker/cagent](https://github.com/docker/cagent) ([v1.9.30](https://github.com/docker/cagent/releases/tag/v1.9.30), 1 day ago) - Agent Builder and Runtime by Docker Engineering
- [docker/packaging](https://github.com/docker/packaging) ([containerd/v2.2.0-54](https://github.com/docker/packaging/releases/tag/containerd/v2.2.0-54), 1 day ago) - Docker Packaging (deb, rpm, static)
- [crazy-max/docker-matomo](https://github.com/crazy-max/docker-matomo) ([5.6.1-r0](https://github.com/crazy-max/docker-matomo/releases/tag/5.6.1-r0), 1 day ago) - Matomo (formerly Piwik) Docker image
- [moby/moby](https://github.com/moby/moby) ([docker-v29.1.1](https://github.com/moby/moby/releases/tag/docker-v29.1.1), 1 day ago) - The Moby Project - a collaborative project for the container ecosystem to assemble container-based systems
- [goreleaser/goreleaser](https://github.com/goreleaser/goreleaser) ([nightly](https://github.com/goreleaser/goreleaser/releases/tag/nightly), 1 day ago) - Release engineering, simplified

#### ❤️ Latest sponsors
- [Fedor Dikarev](https://github.com/fedordikarev) (1 month ago)
- [Jonathan Prusik](https://github.com/jprusik) (3 months ago)
- [Don Pattee](https://github.com/DPattee) (6 months ago)
- [Seow Alex](https://github.com/seowalex) (7 months ago)
- [Matthew Fuller](https://github.com/mathematics333) (11 months ago)

Thanks again for your support, it is much appreciated! 🙏
